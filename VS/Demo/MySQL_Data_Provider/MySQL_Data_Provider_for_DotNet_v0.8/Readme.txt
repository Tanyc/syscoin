...
Getting started
---------------
Open the C# project and compile it. This creates the DLL.

I have included the libmysql.dll, the library with the C-API
functions, that MySQLClient uses. Either copy it beside the 
new compiled DLL (into Bin\Debug) or into the system directory.
This version is taken from 4.0.3 beta.

Note: If you use older versions of the libmysql.dll, the data provider
might crash because of a different definition of the mysql_field struct
in the C-API.
I could not find out, with which version this change was introduced.
I have included further notes in the code at the place where it crashes,
so it will be easy for you to do the changes (just comment out one line).

Additionally, I have included a small code example. It shows the
use of the DataAdapter, Unicode, Blobs and Transactions.
Before you run the example, create a new database in MySQL and
import the "codeexample.sql" file.
