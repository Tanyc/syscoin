...
class UploadFile {//类定义开始
    ...
    public function upload($savePath ='') {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                ...
                $file['savename']   =   $this->genSaveName($file,$index++);
     ...
     * 自定义上传文件名称
     * @access private
     * @param string $filename：上传文件信息， $index:编号
     * @return string 保存文件名
     */
    private function genSaveName($file,$index) {
        $extension = $file['extension']; 
        $UID = $_SESSION[C('USER_AUTH_KEY')];
        $filename = $UID . time() . $index . "." . $extension;
        return $filename;
    }
